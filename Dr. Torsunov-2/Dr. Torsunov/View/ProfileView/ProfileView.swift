...
struct ProfileView: View {
    ...
    var body: some View {
        ZStack(alignment: .top) {
            ...
            VStack {
                ...
                VStack {
                    HStack {
                        if showPersonalView {
                            Button {
                                withAnimation { showPersonalView = false }
                            } label: {
                    ...
                    VStack(spacing: 0) {
                        ...
                        if showPersonalView {
                            ...
                            profileRow(icon: "person.fill", title: "Личные данные") {
                                withAnimation { showPersonalView = true }
                            }
                            Divider().background(Color.gray.opacity(0.5))
            ...
            VStack {
                ...
                ZStack(alignment: .bottomTrailing) {
                    ...
                    } else if viewModel.isLoadingAvatar {
                        ProgressView()
                            .frame(width: 100, height: 100)
                    } else {
        ...
        .onAppear {
            showContent = true
            viewModel.loadUserAndAvatar()
        }
        .sheet(isPresented: $viewModel.showPhotoPicker) {
            ImagePicker(image: Binding(
                get: { nil },
                set: { image in
                    if let image { viewModel.setPhoto(image) }
                }
            ))
        }
    }
